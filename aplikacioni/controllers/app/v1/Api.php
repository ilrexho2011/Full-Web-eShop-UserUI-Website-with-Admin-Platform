    ...
    */
    ...
    public function get_orders()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $user_id = isset($this->user_details['id']) && $this->user_details['id'] !== null ? $this->user_details['id'] : '';            $multiple_status = (isset($_POST['active_status']) && !empty($_POST['active_status'])) ? explode(',', $_POST['active_status']) : false;
                ...
                if (!empty($tmp_cart_user_data)) {
                    for ($i = 0; $i < count($tmp_cart_user_data); $i++) {
                        ...
                        if (!empty($product_data[0]['product_id'])) {
                            ...
                            $additional_product_details = fetch_details('products', ['id' => $product_data[0]['product_id']], 'minimum_order_quantity,total_allowed_quantity');
    ...
    public function delete_user()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            if ($user_data) {
                $login = $this->ion_auth->login($this->input->post('mobile'), $this->input->post('password'), false);                if ($login) {
    ...
    */
    ...
    public function delete_social_account()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->form_validation->set_rules('id', 'user id', 'trim|xss_clean');
            $user_id = isset($this->user_details['id']) && $this->user_details['id'] !== null ? $this->user_details['id'] : '';            $user_data = fetch_details('users', ['id' => $user_id], 'id,username');
            if ($user_data) {
                $user_group = fetch_details('users_groups', ['user_id' => $user_id], 'group_id');
                if ($user_group[0]['group_id'] == '2') {
                    $status = 'awaiting,received,processed,shipped';
